...
class Registry {
  ...
  addComponent(entity, ...components) {
    ...
    if (!identifiedArchtype) {
      // create new archtype
      identifiedArchtype = new Archtype(ctorsSet, this._maxEntityCount);
...
class Archtype {
  ...
  match(componentCtorSet, exact = true) {
    // if (!componentCtorSet || componentCtorSet.length === 0) {
    //   throw new Error(
    ...
    // let matched = 0;
    // for (let ctor of componentCtorSet) {
    //   if (!this._componentCtorSet.has(ctor)) {
    //     return false;
    //   }
    //   matched++;
    // }
    // if (exact && matched !== this._componentCtorSet.size) return false;
    // return true;
